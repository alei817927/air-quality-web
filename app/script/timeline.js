...
function SetProgressTime(fun, startTime, endTime) {
  ...
  ScrollBar.maxValue = Math.abs(qdsjDate - jssjDate) / 3600000;
  //初始化
...
function progressTimeControl(img) {
  if ($(img).attr("title") == "暂停") {
    $(img).attr("title", "开始");
    $(img).css("background-image", "url(/app/images/play.png)");
    window.clearInterval(_mProgressTimer);
  }else {
    $(img).attr("title", "暂停");
    $(img).css("background-image", "url(/app/images/pause.png)");
...
function progressTimeStop() {
  ...
  $("#progressTime_control").css("background-image", "url(/app/images/play.png)");
  $("#scroll_Thumb").css("margin-left", "0px");
  $("#scroll_Track").css("width", "0px");
  ScrollBar.value = 0;
  _index = 0;
